...
`seq` 是同一个 `runId` 内的 SSE 事件序号，建议从 1 开始单调递增。它不用于 UI 展示，主要用于排查流式事件乱序、重复、丢失，以及对齐前后端日志。

示例：

```text
runId = run_001
seq = 1  STREAM_CREATED
seq = 2  MESSAGE_STARTED
seq = 3  ANSWER_DELTA
seq = 4  ANSWER_DELTA
seq = 5  STREAM_COMPLETED
```

如果前端收到 `seq = 1, 2, 4, 5`，说明 `seq = 3` 可能丢失；如果收到两个 `seq = 4`，说明事件可能重复。
